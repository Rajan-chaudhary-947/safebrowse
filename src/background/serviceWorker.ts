import type { ActivityEvent, Policy } from '../common/types';
...
function policyRulePriority(policy: Policy, target: string): number {
...
function policyUrlFilter(policy: Policy, target: string): string {
  return policy.type === 'DOMAIN' || policy.type === 'CATEGORY'
    ? domainRuleFilter(target)
    : urlPrefixFilter(target);
}

function activePolicy(policy: Policy, now: Date): boolean {
  return policy.enabled && isTimeInSchedule(now, policy.schedule);
}

async function createDynamicRules(policies: Policy[]): Promise<number> {
  const state = await getInternalState();
  const existingIds = await getCurrentRuleIds();

  if (!state.settings.protectionEnabled) {
    if (existingIds.length) await chrome.declarativeNetRequest.updateDynamicRules({ removeRuleIds: existingIds });
    return 0;
  }
      ...
      if (policy.action === 'BLOCK') {
        rules.push({
          ...
          action: {
            type: 'redirect',
            redirect: {
              url: `${chrome.runtime.getURL('blocked.html')}?site=${encodeURIComponent(
                target
...
async function requireAdmin(): Promise<void> {
  const state = await getInternalState();
  if (!(await isAdminUnlocked(state.settings))) throw new Error('Parent authentication required.');
...
async function recordBlockedPage(
  originalUrl: string,
  policyId?: string,
  profileId?: string
): Promise<{ ok: boolean }> {
  ...
  const policies =
    state.profileData[targetProfileId]?.policies ?? [];
  ...
  await appendEvent({
    ...
    detail:
      'Top-level navigation redirected to SafeBrowse blocked page'
  });
...
async function handleMessage(message: Message): Promise<unknown> {
  ...
  if (message.type === 'VERIFY_PIN') {
    const state = await getInternalState();
    if (!state.settings.pinHash) return { ok: true, alreadyUnlocked: true };
    validatePin(message.pin);
    const ok = await verifyPin(message.pin, state.settings.pinHash);
    if (ok) await setAdminUnlocked(true);
    return { ok };
  }
  ...
  if (message.type === 'SET_PIN') {
    ...
    if (state.settings.pinHash) {
      await requireAdmin();
      if (!message.currentPin || !(await verifyPin(message.currentPin, state.settings.pinHash))) throw new Error('Current PIN is incorrect.');
    }
    validatePin(message.pin);
  ...
  if (message.type === 'CREATE_PROFILE') {
    ...
    if (!profile.name) throw new Error('Profile name cannot be empty.');
  ...
  if (message.type === 'SWITCH_PROFILE') {
    await requireAdmin();
    const state = await getInternalState();
    if (state.activeProfileId === message.profileId) return getState();
  ...
  if (message.type === 'SET_RETENTION') {
    await requireAdmin();
    if (![7, 14, 30, 90].includes(message.retentionDays)) throw new Error('Invalid retention period.');
    const state = await getInternalState();
  ...
  if (message.type === 'IMPORT_BACKUP') {
    await requireAdmin();
    if (!validateBackup(message.bundle)) throw new Error('Invalid SafeBrowse backup file.');
  ...
  if (message.type === 'SET_SYNC_ENABLED') {
    ...
    if (message.enabled) await pushSync();
    return getState();
  }
  ...
  if (message.type === 'SYNC_PULL') {
    ...
    if (!updated) throw new Error('No newer SafeBrowse configuration was found in Chrome Sync.');
...
chrome.runtime.onInstalled.addListener(async () => {
  ...
  if (raw.settings.syncEnabled) await pullSyncIfNewer(raw);
  await rebuild();
});
