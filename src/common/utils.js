...
export function normalizeDomain(input) {
    ...
    if (!hostname || !hostname.includes('.') || hostname.includes(' '))
        throw new Error('Enter a valid domain such as youtube.com.');
    return hostname;
}
export function normalizeUrlPattern(input) {
    ...
    if (!['http:', 'https:'].includes(url.protocol))
        throw new Error('Only HTTP(S) URL patterns are supported.');
    return value;
}
export function isTimeInSchedule(now, schedule) {
    if (!schedule.enabled)
        return true;
    ...
    if (start < end)
        return schedule.days.includes(now.getDay()) && current >= start && current < end;
...
export function expandPolicyTargets(policy) {
...
export function policyMatchesUrl(policy, inputUrl, now = new Date()) {
    if (!policy.enabled || !isTimeInSchedule(now, policy.schedule))
        return false;
    let url;
    try {
...
export function policySpecificity(policy, target) {
    if (policy.type === 'URL_PATTERN')
        return 10_000 + Math.min(9_000, (target ?? policy.targets[0] ?? '').length);
    if (policy.type === 'DOMAIN')
        return 1_000 + Math.min(900, (target ?? '').split('.').filter(Boolean).length * 50);
    return 100;
}
export function resolveWinningPolicy(policies, inputUrl, now = new Date()) {
    ...
    matches.sort((a, b) => {
        ...
        if (a.action !== b.action)
            return a.action === 'ALLOW' ? -1 : 1;
        return b.updatedAt - a.updatedAt;
    });
    return matches[0] ?? null;
}
export function resolvePolicyExplanation(policies, inputUrl, now = new Date()) {
...
export function domainRuleFilter(domain) {
    return `||${normalizeDomain(domain)}/`;
}
export function urlPrefixFilter(value) {
    return `|${value.replace(/([\\|*^])/g, '\\$1')}`;
}
export function escapeUrlFilter(value) {
